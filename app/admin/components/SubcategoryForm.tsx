...
export default function SubcategoryForm({ subcategory, categories }: { subcategory?: any, categories: any[] }) {
  ...
  const [formData, setFormData] = useState({
    ...
    categoryId: subcategory?.categoryId || (categories.length > 0 ? categories[0]._id : ''),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = subcategory ? `/api/admin/subcategories/${subcategory._id}` : '/api/admin/subcategories';
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <div className={styles.inputGroup}>
          ...
          <ImageUploader 
            ...
            folder="yourstore/subcategories"
            multiple={false}
          />
        </div>
